...
---
Components
1. Remote Hydrometer — `dam\_remotehyd` (RH)
...
`PRE\_ALARM`	12 cm < distance ≤ 20 cm	Pulsing
...
Communication
On startup, the ESP32 connects to the WiFi network and sends its timing parameters (`prealarm\_time`, `alarm\_time`) to the Dam Service. Afterwards, depending on the detected state, it sends JSON payloads via `POST /api/data` containing:
...
---
2. Dam Service — `dam\_service` (DS)
Platform: PC / Server  
Language: Java  
Framework: Vert.x 4 (reactive Verticle architecture)  
...
---
5. Dam Dashboard — `dam\_dashboard` (DD)
